...
Функция ПолучитьДом(стрДома,ДатаОткрытия)
	...
	Если Не ЗначениеЗаполнено(ДомСсылка) Тогда
		...
		Если ДомСсылка.Пустая() Тогда 
			...
			СтрокаТЧ.Свойство = стрДома.СвойствоМКД;
			СтрокаТЧ.Значение = стрДома.ЗначениеМКД;
			
			СтрокаТЧ = ДомОбъект.ДополнительныеРеквизиты.Добавить();
			СтрокаТЧ.Свойство = стрДома.СвойствоИндекс;
			СтрокаТЧ.Значение = стрДома.Индекс;
...
Функция ПолучитьКонстантыОбработки()
	...
	КонстантыОбработки.Вставить("ВидРасчетаГВС",Справочники.ркВидыРасчетов.НайтиПоНаименованию("ГВС",Истина));
	КонстантыОбработки.Вставить("ВидРасчетаОтопление",Справочники.ркВидыРасчетов.НайтиПоНаименованию("Отопление",Истина));
	...
	КонстантыОбработки.Вставить("КвМетр",Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("055",Истина));
	...
	КонстантыОбработки.Вставить("ГКал",Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду("233",Истина));
	...
	КонстантыОбработки.Вставить("Поставщик",Справочники.Контрагенты.НайтиПоНаименованию("Астраханские тепловые сети",Истина));
	...
	КонстантыОбработки.Вставить("СвойствоВидЭнергоназначений",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Вид энергоназначений"));
	КонстантыОбработки.Вставить("СвойствоТипПрибораУчета",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Тип прибора учета",Истина));
	КонстантыОбработки.Вставить("СвойствоИндекс",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Почтовый индекс",Истина));
	КонстантыОбработки.Вставить("СвойствоЭтажность",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Этажность",Истина));
	...
	КонстантыОбработки.Вставить("МКДНормы",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("МКД",Истина));
	КонстантыОбработки.Вставить("МОПНормы",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("МОП",Истина));
	КонстантыОбработки.Вставить("СвойствоНомерДоговора",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Номер договора",Истина));
	КонстантыОбработки.Вставить("СвойствоОтдельныйВвод",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Отдельный ввод",Истина));
	...
	КонстантыОбработки.Вставить("СвойствоУчаствуетВМОП",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Участвует в МОП",Истина));
	КонстантыОбработки.Вставить("СвойствоНомерПломбы",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Номер пломбы",Истина));
	КонстантыОбработки.Вставить("СвойствоОтключен",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Отключен",Истина));
	КонстантыОбработки.Вставить("СвойствоДатаУстановкиСчетчика",ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Дата установки",Истина));
...
Процедура ВосстановитьЗначения(пАдрес)
	...
	Для Каждого стрДомаТЗ Из ДомаТЗ Цикл
		...
		стрДома = Новый Структура("Наименование,Улица,Корпус,Номер,ДатаПостановкиНаУчет,ДатаСнятияСУчета,СвойствоИндекс,Индекс,СвойствоМКД,ЗначениеМКД");
		ЗаполнитьЗначенияСвойств(стрДома,стрДомаТЗ);
		стрДома.Номер 			= СокрЛП(стрДомаТЗ.НомерДома);
		стрДома.Наименование 	= СокрЛП(стрДомаТЗ.Наименование);
		стрДома.Улица 			= СокрЛП(стрДомаТЗ.Улица);
		стрДома.Корпус 			= СокрЛП(стрДомаТЗ.Корпус);
		стрДома.Индекс 			= СокрЛП(стрДомаТЗ.Индекс);
		стрДома.СвойствоМКД 	= КонстантыОбработки.МКДНормы;
		стрДома.ЗначениеМКД 	= (стрДомаТЗ.ДопРеквизиты.Количество()>0);
		стрДома.СвойствоИндекс 	= КонстантыОбработки.СвойствоИндекс;
		
		ДомСсылка = Справочники.ркДома.НайтиПоНаименованию(стрДома.Наименование,Истина);
		ЭтоНовыйДом = ДомСсылка.Пустая();
		текДом = ПолучитьДом(стрДома,ДатаОткрытия);
		ДомОбъект = текДом.ПолучитьОбъект();
		ЕстьИзменения = Ложь;
		
		текМОП = 0;
		ЕстьДатаИзменения = Ложь;
		Если стрДОмаТЗ.ДопРеквизиты.Колонки.Найти("Период")<>Неопределено Тогда
		...
		Если ЭтоНовыйДом Тогда
			Для Каждого стрДополнительныеРеквизитыТЗ Из стрДомаТЗ.ДопРеквизиты Цикл
				...
				Если НЕ текСвойство.Пустая() И Не текЗначение = Неопределено Тогда
					...
					Иначе
						текДатаИзменения = ?(ЕстьДатаИзменения,стрДополнительныеРеквизитыТЗ.Период,НачалоГода(ДатаОткрытия));
			...
			//НоваяСтрокаДок.Объект = текДом;
			//НоваяСтрокаДок.НачалоДействия = текДатаИзменения;
			//НоваяСтрокаДок.Свойство = СвойствоИндекс;
			//НоваяСтрокаДок.Значение = стрДома.Индекс;
		...
		Для Каждого стрЛицевыеСчетаТЗ Из стрДомаТЗ.ЛС Цикл
			...
			ЛССсылка = Справочники.ркЛицевыеСчета.НайтиПоКоду(СокрЛП(стрЛицевыеСчетаТЗ.НомерЛС));
			Если НЕ ЛССсылка.Пустая() Тогда
				Продолжить;
			КонецЕсли;
			
			ПомещениеСсылка = Справочники.ркПомещения.НайтиПоНаименованию(стрЛицевыеСчетаТЗ.ПомещениеНомер,Истина,,текДом);
			Если ПомещениеСсылка.Пустая() Тогда
				ПомещениеОбъект = Справочники.ркПомещения.СоздатьЭлемент();
				ПомещениеОбъект.Наименование = стрЛицевыеСчетаТЗ.ПомещениеНомер;
				...
				ПомещениеОбъект.ЖилоеМуниципальное = стрЛицевыеСчетаТЗ.ПомещениеЖилоеМуниципальное;
			...
			ЛицевойСчетОбъект.Наименование = стрЛицевыеСчетаТЗ.ОтветственныйКвартиросъемщик;
			...
			ФизЛицо.Наименование = стрЛицевыеСчетаТЗ.ОтветственныйКвартиросъемщик;
			ФизЛицо.ФИО = стрЛицевыеСчетаТЗ.ОтветственныйКвартиросъемщик;
			...
			Для Каждого стрСчетчики Из стрЛицевыеСчетаТЗ.Счетчики Цикл
				...
				Если НЕ ЗначениеЗаполнено(НовыйСчетчик.Наименование) Тогда
					НовыйСчетчик.Наименование = стрСчетчики.Услуга +" "+текДом.Наименование+", "+ПомещениеСсылка.Тип.СокращенноеНаименование + " "+ПомещениеСсылка.Наименование;
				КонецЕсли;
				
				НовыйСчетчик.Номер = стрСчетчики.Номер;
				НовыйСчетчик.Уровень = Справочники.ркУровниСчетчиков.Индивидуальный;
				Если стрСчетчики.Услуга = "Горячая вода" Тогда
					тВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
				ИначеЕсли стрСчетчики.Услуга = "Отопление" Тогда
					тВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
				КонецЕсли;
				НовыйСчетчик.ОсновнойВидРасчета = тВидРасчета;
				...
				СтрокаТЧ.ВидРасчета = тВидРасчета;
				...
				Если стрСчетчики.Услуга = "Горячая вода" Тогда
					стрВидыПоказаний.ЕдиницыУчета = КонстантыОбработки.КубМетр;
					тВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
				ИначеЕсли стрСчетчики.Услуга = "Отопление" Тогда
					стрВидыПоказаний.ЕдиницыУчета = КонстантыОбработки.ГКал;
					тВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
				КонецЕсли;
				стрВидыПоказаний.НачальныеПоказания = стрСчетчики.НачальныеПоказания;
				...
				стрДокИзмИПУ.НачалоДействия = стрСчетчики.ДатаУстановки;
				стрДокИзмИПУ.Счетчик = НовыйСчетчик.Ссылка;
				...
				Если КонстантыОбработки.СвойствоДатаПоверки.Периодический Тогда
					...
					стрДокИзм.Объект = НовыйСчетчик.Ссылка;
				...
				Для Каждого стрПоказание Из стрСчетчики.ПоказанияСчетчика Цикл
					...
					НоваяСтрокаТЗ.ВидРасчета = тВидРасчета;
					...
					НоваяСтрокаТЗ.ДатаНачала = НачалоМесяца(стрПоказание.Период);
					НоваяСтрокаТЗ.ДатаОкончания = КонецМесяца(стрПоказание.Период);
					...
					НоваяСтрокаТЗ.Счетчик = НовыйСчетчик.Ссылка;
			...
			Для Каждого стрНачисление Из стрЛицевыеСчетаТЗ.Начисления Цикл
				...
				СтруктураПоиска = Новый Структура("НачалоДействия,ЛицевойСчет,ВидРасчета", НачалоДня(стрНачисление.Период), текЛицевойСчет, текВидТарифа.ВидРасчета);
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					НоваяСтрокаСоставНачислений.ВидРасчета = текВидТарифа.ВидРасчета;
				...
				Если стрНачисление.НаОбщедомовыеНужды Тогда
					...
					НоваяЗаписьНачислений.ВидРасчета = текВидТарифа.ВидРасчета;
		...
		Если ЭтоНовыйДом Тогда
			Для Каждого стрОдпуТЗ Из стрДомаТЗ.ОДПУ Цикл
				...
				Если стрОдпуТЗ.Услуга = "Горячая вода" Тогда
					тВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
				ИначеЕсли стрОдпуТЗ.Услуга = "Отопление" Тогда
					тВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
				КонецЕсли;
				СчетчикОбъект.ОсновнойВидРасчета = тВидРасчета;
				...
				Для Каждого текВидПоказаний Из МассивВидовПоказаний Цикл
					...
					стрВидыРасчетов.ВидРасчета = тВидРасчета;
					...
					стрВидыПоказаний.НачальныеПоказания = стрОдпуТЗ.НачальныеПоказания;
				...
				стрДокИзмГПУ.Включен = Истина;
				стрДокИзмГПУ.НачалоДействия = стрОдпуТЗ.ДатаВключения;
				стрДокИзмГПУ.Счетчик = СчетчикОбъект.Ссылка;
...
Процедура ЗаписатьПоказания(пТЗ,КонстантыОбработки)
	...
	Для Каждого СтрокаТЗ ИЗ пТЗ Цикл
		...
		НоваяСтрокаТЧ.ВидРасчета = СтрокаТЗ.ВидРасчета;
...
Процедура ВосстановитьНормыИТарифы(пАдрес)
	...
	Для Каждого стрНормыИТарифыТЗ Из НормыИТарифыТЗ Цикл
		...
		текЕдиницаИзмерения = Неопределено;
		
		Если стрНормыИТарифыТЗ.Услуга = "Отопление" Тогда
			тВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
			тЕдиницаИзмеренияИсходныхДанных = КонстантыОбработки.КвМетр;
			тЕдиницаИзмерения = КонстантыОбработки.ГКал;
		ИначеЕсли стрНормыИТарифыТЗ.Услуга = "Горячая вода" ИЛИ стрНормыИТарифыТЗ.Услуга = "Горячая вода (ОДН)" Тогда
			
			тВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
			тЕдиницаИзмеренияИсходныхДанных = КонстантыОбработки.Человек;
			тЕдиницаИзмерения = КонстантыОбработки.КубМетр;

			
			//единица измерения исходных данных = чел
			//единица измерения результата = м3
			
			//вид расчета = теплоноситель
			//вид расчета = теплоэнергия
		КонецЕсли;
		текНаименование = ?(ЗначениеЗаполнено(стрНормыИТарифыТЗ.Наименование),стрНормыИТарифыТЗ.Наименование,"Безымянный");								
		текВидНормы = Справочники.ркВидыНорм.НайтиПоНаименованию(текНаименование,Истина);
		текВидТарифа = Справочники.ркВидыТарифов.НайтиПоНаименованию(текНаименование,Истина);
		...
		Если текВидНормы.Пустая() Тогда
			...
			НовыйВидНормы.ЕдиницаИзмеренияИсходныхДанных = тЕдиницаИзмеренияИсходныхДанных;
			НовыйВидНормы.ЕдиницаИзмерения = тЕдиницаИзмерения;
			НовыйВидНормы.Наименование = текНаименование;
		...
		Если текВидТарифа.Пустая() Тогда
			...
			НовыйВидТарифа.ВидРасчета = тВидРасчета;
			НовыйВидТарифа.Наименование = текНаименование;
			НовыйВидТарифа.ЕдиницыУчета = тЕдиницаИзмерения;
			НовыйВидТарифа.ВидНормы = текВидНормы;
...
Процедура ВосстановитьНежилые(пАдрес)
	...
	Запрос.Текст = 
		...
		|		ПО втДома.Наименование = ркЛицевыеСчета.Наименование
		|			И втДома.НайденныйДом = ркЛицевыеСчета.Дом
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ркСчетчики.Ссылка КАК НайденныйСчетчик,
		|	втАбоненты.КодНП КАК КодНП,
		|	втАбоненты.Услуга,
		|ИЗ
		|	втАбоненты КАК втАбоненты
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ркСчетчики КАК ркСчетчики
		|		ПО втАбоненты.НайденныйЛС.Помещение = ркСчетчики.Владелец
		|			И втАбоненты.ЗаводскойНомер = ркСчетчики.Номер
		|ИТОГИ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(Выборка.НайденныйДом) Тогда
			...
			//ДомСсылка = Справочники.ркДома.НайтиПоНаименованию(стрДома.Наименование,Истина);
			//ЭтоНовыйДом = ДомСсылка.Пустая();
			//текДом = ПолучитьДом(стрДома,ДатаОткрытия,МКДНормы,стрДомаТЗ.ДопРеквизиты.Количество()>0);
		...
		Если НЕ ЗначениеЗаполнено(тАбонент) Тогда
			...
			//СтрокаДопРеквизитов = НовыйАбонент.ДополнительныеРеквизиты.Добавить();
			//СтрокаДопРеквизитов.Свойство = СвойствоПлощадь;
		...
		Пока ВыборкаСчетчики.Следующий() Цикл
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаСчетчики.НайденныйСчетчик) Тогда
				...
				СтрокаТЧ = НовыйСчетчик.ДополнительныеРеквизиты.Добавить();
				СтрокаТЧ.Свойство = КонстантыОбработки.СвойствоДатаУстановкиСчетчика;
				...
				СтрокаТЧ = НовыйСчетчик.ВидыРасчетов.Добавить();
				СтрокаТЧ.ВидПоказаний = Справочники.ркВидыПоказанийСчетчиков.Обычные;
				Если ВыборкаСчетчики.Услуга = "Горячая вода" Тогда
					СтрокаТЧ.ВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
				ИначеЕсли ВыборкаСчетчики.Услуга = "Отопление" Тогда
					СтрокаТЧ.ВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
				КонецЕсли;
				СтрокаТЧ.ЕдиницаРасчетаНормативногоКоличества = КонстантыОбработки.ЕдиницаКоличествоПроживающих;
...
Процедура ЗагрузитьНачисленияНежилыхНаСервере(пАдрес)
	...
	//	|	ркЛицевыеСчета.Ссылка КАК НайденныйАбонент,
	//	|	ркСчетчики.Ссылка КАК НайденныйПУ,
	//	|	ркЛицевыеСчета1.Ссылка КАК АбонентСчетчика,
	//	|	ркЛицевыеСчета.Дом.УК
	//	|ИЗ
	//	|	втТЗ КАК втТЗ
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ркЛицевыеСчета КАК ркЛицевыеСчета
	//	|		ПО втТЗ.НежилоеПомещениеНаименование = ркЛицевыеСчета.Наименование
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ркСчетчики КАК ркСчетчики
	//	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ркЛицевыеСчета КАК ркЛицевыеСчета1
	//	|			ПО ркСчетчики.Владелец = ркЛицевыеСчета1.Помещение
	//	|		ПО втТЗ.ПриборУчетаНаименование = ркСчетчики.Наименование
	//	|			И втТЗ.НежилоеПомещениеНаименование = ркСчетчики.Владелец.Наименование
	//	|ГДЕ
	//	|	(ркЛицевыеСчета.Ссылка = ркЛицевыеСчета1.Ссылка
	//	|			ИЛИ ркСчетчики.Ссылка ЕСТЬ NULL )";
	//	
...
Процедура ЗагрузитьНормыОДННаСервере(пАдрес)
	...
	//РезультатЗапроса = Запрос.Выполнить();
	//
	//ВыборкаДом = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	//
	//Пока ВыборкаДом.Следующий() Цикл
	//	тДом = ВыборкаДом.Ссылка;
	//	НЗ = РегистрыСведений.рцПараметрыРасчетаОДН.СоздатьНаборЗаписей();
	//	НЗ.Отбор.Дом.Установить(тДом);
	//	НЗ.Отбор.ВидРасчета.Установить(КонстантыОбработки.ВидРасчетаЭлектроэнергия);
	//	НЗ.Прочитать();
	//	Выборка = ВыборкаДом.Выбрать();
	//	Пока Выборка.Следующий() Цикл			
	//		тЗапись = НЗ.Добавить();
	//		тЗапись.ВидРасчета = КонстантыОбработки.ВидРасчетаЭлектроэнергия;
	//		тЗапись.Дом = тДом;
	//		тЗапись.ЗначениеНормы = Выборка.Значение;
	//		тЗапись.ОграничиватьПоказанияОДПУ = Истина;
	//		тЗапись.Период = Выборка.Период;
	//	КонецЦикла;
	//	НЗ.Записать(Истина);
	//КонецЦикла;
	//
...
Процедура ДоЗаписатьПоказания(пТЗ,КонстантыОбработки)
	...
	Запрос.Текст = 
		...
		|	втТЗ.Дом,
		|	втТЗ.ЛицевойСчет,
		|	МАКСИМУМ(втТЗ.КонечныеПоказания) КАК КонечныеПоказания,
		|	втТЗ.ПериодДействия,
		|	втТЗ.Помещение,
		|	втТЗ.Счетчик,
		|	втТЗ.ВидПоказаний
		...
		|	рцПоказанияСчетчиковСрезПоследних.НачальныеПоказания,
		...
		|	втМакс.Счетчик,
		|	втМакс.ВидПоказаний,
		|	ЕСТЬNULL(втСрезМаксПоказания.НачальныеПоказания, 0) КАК НачальноеКоличество
		|ИЗ
		|	втМакс КАК втМакс
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Услуга = "Горячая вода" Тогда
			НоваяСтрокаТЧ.ВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
			НоваяСтрокаТЧ.ЕдиницаИзмерения = КонстантыОбработки.КубМетр;
		ИначеЕсли ВыборкаДетальныеЗаписи.Услуга = "Отопление" Тогда
			НоваяСтрокаТЧ.ВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
			НоваяСтрокаТЧ.ЕдиницаИзмерения = КонстантыОбработки.ГКал;
		КонецЕсли;
...
Процедура ДогрузитьОплатыСервер(пАдрес)
	...
	Для каждого СтрокаТЗ Из ТЗ Цикл
		...
		Если СтрокаТЗ.Услуга = "Горячая вода" Тогда
			НоваяСтрокаСоставОплаты.ВидРасчета = КонстантыОбработки.ВидРасчетаГВС;
		ИначеЕсли СтрокаТЗ.Услуга = "Отопление" Тогда
			НоваяСтрокаСоставОплаты.ВидРасчета = КонстантыОбработки.ВидРасчетаОтопление;
		КонецЕсли;
		НоваяСтрокаСоставОплаты.Поставщик = КонстантыОбработки.Поставщик;
