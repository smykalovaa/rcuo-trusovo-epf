...
Процедура ВосстановитьЗначения(пАдрес)
	...
	Для Каждого стрДомаТЗ Из ДомаТЗ Цикл
		...
		Для Каждого стрДомУК Из стрДомаТЗ.ТзУК Цикл
			...
			НоваяСтрокаТЧ.НачалоДействия = стрДомУК.НачалоДействия;
		...
		Для Каждого стрЛицевыеСчетаТЗ Из стрДомаТЗ.ЛС Цикл
			...
			Для Каждого стрСчетчики Из стрЛицевыеСчетаТЗ.Счетчики Цикл
				...
				Если текСчетчик.Пустая() Тогда
					...
					Для Каждого стрПоказание Из стрСчетчики.ПоказанияСчетчика Цикл
						...
						Если стрПоказание.Переворот Тогда
							НоваяСтрокаТЗ.Количество = ПолучитьКоличествоПереворот(стрПоказание.КонечноеПоказание,стрПоказание.НачальноеПоказание);
						Иначе
			...
			Для Каждого стрСальдо Из стрЛицевыеСчетаТЗ.Сальдо Цикл
				...
				НоваяЗапись.Период = ДатаОграничения;
			...
			Для Каждого стрНачисление Из стрЛицевыеСчетаТЗ.Начисления Цикл
				...
				Иначе
					...
					НоваяЗаписьНачислений.УК = текДом.УК;
				КонецЕсли;
				Если стрНачисление.Среднее = "Среднее" Тогда
					НоваяЗаписьНачислений.ВидПерерасчета = КонстантыОбработки.ВидПерерасчетаСреднее;
				ИначеЕсли стрНачисление.Среднее = "Среднее по нормативу" Тогда
...
Процедура ЗаписатьПоказания(пТЗ,КонстантыОбработки)
	...
	пТЗ.Сортировать("ПериодДействия Возр,КодСчетчика Возр");
	ДокПоказанияСчетчиков = Неопределено;
	...
	Для Каждого СтрокаТЗ ИЗ пТЗ Цикл
		Если СтрокаТЗ.ОтчетныйПериод >= ДатаОграничения Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого стрНормыИТарифыТЗ Из НормыИТарифыТЗ Цикл
		...
		Если текГруппаВидаНормы.Пустая() Тогда
			...
			Если текВидТарифа.Пустая() Тогда
				...
				Если стрНормыИТарифыТЗ.СпособРасчета = "По количеству человек" Тогда
					СтрокаТЧ.ЕдиницаРасчета = КонстантыОбработки.ЕдиницаКоличествоПроживающих;
				ИначеЕсли стрНормыИТарифыТЗ.СпособРасчета = "По приборам учета" Тогда
...
Процедура ДоЗаписатьПоказания(пТЗ,КонстантыОбработки)
	...
	Запрос.Текст = 
		...
		|	&ТЗ КАК ТЗ
		|ГДЕ
		|	НАЧАЛОПЕРИОДА(ТЗ.ПериодДействия, МЕСЯЦ) = &ДатаЗагрузки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
